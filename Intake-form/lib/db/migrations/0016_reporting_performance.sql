-- 0016_reporting_performance.sql — make the journey metric fast.
--
-- MEASURED, not guessed. On production data:
--     drsnip_journey_metric          6,340 ms  and  5,996 ms
--     drsnip_journey_metric (appt)      23 ms
--     drsnip_booking_metric             21 ms
--     drsnip_journey_freshness           4 ms
--
-- EXPLAIN ANALYZE showed why: the correlated subquery that finds each
-- patient's first later submission ran as `SubPlan 2 ... loops=1845`, a full
-- sequential scan of `submissions` per patient. Two causes, both fixed here:
--
--   1. `submissions` had NO index on n8n_patient_id at all.
--   2. The function compared `c.n8n_patient_id::text = fe.pid`. Casting the
--      COLUMN makes any btree index on it unusable, so adding an index alone
--      would have changed nothing. The CTE now keeps pid as bigint and casts
--      only where it must join a text column.
--
-- The page issues three of these calls, so this was roughly 18 seconds of
-- database time on every load.
--
-- Measured on production, same connection style, two runs each so neither
-- number is a cold/warm artefact:
--   before: 6340 ms, 5996 ms   after: 57 ms, 38 ms   (registration_to_consultation)
--   before:   23 ms            after: 19 ms          (appointment_evidence_registration)
-- Every metric value is byte-identical before and after.
--
-- Additive and idempotent. Safe to re-run; not registered in migrate.ts
-- because it depends on the functions created by 0014.

-- ---------------------------------------------------------------------------
-- 1. The index the correlated lookup needs.
--
--    Partial: rows with no patient id are never looked up this way. Ordered
--    (patient, form_type, created_at) so the planner can satisfy
--    "this patient's earliest <form> after <instant>" from the index alone.
-- ---------------------------------------------------------------------------
CREATE INDEX IF NOT EXISTS submissions_patient_form_created_idx
  ON public.submissions (n8n_patient_id, form_type, created_at)
  WHERE n8n_patient_id IS NOT NULL;

ANALYZE public.submissions;

-- ---------------------------------------------------------------------------
-- 2. The same function as 0014, with the column casts removed.
--    Everything else is byte-identical, deliberately: this is a performance
--    fix, not a definition change, and the numbers it returns must not move.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  IF p_metric = 'registration_to_consultation'
    ...
    WITH first_entry AS (
      SELECT s.n8n_patient_id AS pid, min(s.created_at) AS entry_at
    ...
    e AS (
      SELECT fe.pid, fe.entry_at,
             ...
             (SELECT min(r2.created_at) FROM public.submissions r2
               WHERE r2.n8n_patient_id = fe.pid
  ...
  ELSE
    ...
    a AS (
      SELECT e.pid, e.entry_at,
             ...
             s.source_created_at
        FROM e JOIN public.appointment_snapshots s ON s.patient_source_id = e.pid::text
       WHERE s.source_created_at IS NOT NULL
    ),
    pp AS (
      SELECT e.pid, e.entry_at,
             (e.pid::text IN (SELECT patient_source_id FROM hc))        AS hist_complete,
...
ALTER FUNCTION public.drsnip_journey_metric(text, date, date, integer) OWNER TO drsnip_metrics_fn;
...
BEGIN
  ...
  END IF;
  IF has_function_privilege('public','public.drsnip_journey_metric(text,date,date,integer)','EXECUTE') THEN
    RAISE EXCEPTION 'PUBLIC can execute the journey function';
  END IF;
  RAISE NOTICE 'journey metric performance fix applied';
END $$;
