-- 0015_booking_metrics_access.sql — booking evidence through the same boundary.
--
-- A SEPARATE function rather than a wider drsnip_journey_metric(). Changing
-- that function's RETURNS TABLE needs DROP + CREATE, and it is live in
-- production (release v83) — a new function adds capability with no window in
-- which the deployed endpoint has nothing to call.
--
-- Same security posture as 0014, for the same reasons:
--   * owned by drsnip_metrics_fn — NOLOGIN, non-superuser, SELECT on four tables
--   * search_path = pg_catalog, pg_temp; every object schema-qualified;
--     pg_temp LAST so a caller cannot shadow a table with a temporary one
--   * fixed allow-list, bound parameters, no caller text in SQL
--   * suppression applied INSIDE the boundary
--   * statement/lock timeouts; EXECUTE revoked from PUBLIC
--
-- NOT registered in migrate.ts: it creates nothing that a ledger-free replay
-- should re-run, and it needs grants.

-- The status-evidence summary counts transitions, so the function owner needs
-- SELECT on that one table. It is aggregate-only and suppressed, and 0014's
-- assertion was relaxed in step so the two files do not fight on replay.
GRANT SELECT ON TABLE public.appointment_status_transitions TO drsnip_metrics_fn;
...
BEGIN
  ...
  SELECT max(w.completed_at) INTO cutoff
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.drsnip_status_evidence()
RETURNS TABLE (status text, transitions integer, appointments integer, patients integer)
LANGUAGE sql
SECURITY DEFINER
STABLE
SET search_path = pg_catalog, pg_temp
...
AS $$
  SELECT q.status, q.transitions, q.appointments, q.patients
    FROM (
      SELECT coalesce(t.to_status, '')                  AS status,
        ...
        JOIN public.appointment_snapshots s
          ON s.source_appointment_id = t.source_appointment_id
       WHERE t.missing_since IS NULL
       GROUP BY 1
    ) q
   WHERE q.patients >= 5          -- withhold small groups
   ORDER BY q.transitions DESC;
$$;

ALTER FUNCTION public.drsnip_status_evidence() OWNER TO drsnip_metrics_fn;
REVOKE ALL ON FUNCTION public.drsnip_status_evidence() FROM PUBLIC;

DO $$
BEGIN
  FOR i IN 1..1 LOOP
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='drsnip_reporting_ro') THEN
      EXECUTE 'GRANT EXECUTE ON FUNCTION public.drsnip_booking_metric(text,date,date,integer) TO drsnip_reporting_ro';
      EXECUTE 'GRANT EXECUTE ON FUNCTION public.drsnip_status_evidence() TO drsnip_reporting_ro';
    END IF;
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname='drsnip_intake_demo') THEN
      EXECUTE 'GRANT EXECUTE ON FUNCTION public.drsnip_booking_metric(text,date,date,integer) TO drsnip_intake_demo';
      EXECUTE 'GRANT EXECUTE ON FUNCTION public.drsnip_status_evidence() TO drsnip_intake_demo';
    END IF;
  END LOOP;
END $$;
...
BEGIN
  ...
  IF has_function_privilege('public','public.drsnip_booking_metric(text,date,date,integer)','EXECUTE') THEN
    RAISE EXCEPTION 'PUBLIC can execute the booking function';
  END IF;
  RAISE NOTICE 'booking metric boundary verified';
END $$;
