...
import { EMBED_FORMS, iframeSnippet } from "@/lib/embed";
...
// Generated by the same builder as the console's Embed card, so the guide and
// the console can never hand out different snippets.
const REGISTRATION_EMBED_SNIPPET = iframeSnippet(
  EMBED_FORMS.find((f) => f.key === "registration")!,
  "registration-page",
);
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 text-slate-800 print:bg-white">
      ...
      <div className="mx-auto w-full max-w-3xl px-5 py-12 sm:px-8 sm:py-16">
        ...
        <P>
          <B>The registration form now resizes itself when embedded</B>, like the
          insurance form. The fixed-height frame on drsnip.com/registration can go; the
          replacement snippet is in section 3a below.
        </P>
        <P>
        ...
        <P>
          The height message is the same one the insurance form sends, from{" "}
          <C>https://intake.drsnip.com</C>, to <C>https://drsnip.com</C> and{" "}
          <C>https://www.drsnip.com</C> only:
        </P>
        <CodeBlock code={SAMPLE_REGISTRATION_HEIGHT} />
        <DataTable
          head={["Field", "Value"]}
          rows={[
            ["type", "drsnip:height"],
            ["height", "whole number of pixels: the form's full content height"],
          ]}
        />
        <P>
          It is sent when the form loads, on every step, whenever an answer opens or
          closes an extra field, when a card photo is added or removed, and on the
        ...
        <P>
          ...
          snippet includes it. The card upload uses the phone&rsquo;s standard photo
          picker, which offers &ldquo;Take Photo&rdquo; as well as the photo library,
          so it works inside the frame without any further setup.
        </P>
